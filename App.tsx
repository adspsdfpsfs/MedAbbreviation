...
export default function App() {
  ...
  const [allMedicalData, setAllMedicalData] = useState<MedicalTerm[]>(MEDICAL_DATA);
  ...
  useEffect(() => {
    if (query.trim() && localResults.length === 0 && !aiResult && !aiLoading && contextInputRef.current) {
      contextInputRef.current.focus();
    }
  }, [query, localResults.length, aiResult, aiLoading]);

  return (
    <div className="min-h-screen flex flex-col bg-cream relative selection:bg-honey-400 selection:text-navy-900">
      
      {/* Decorative Blobs - Keeping red ONLY in background decoration */}
      ...
      <header className="bg-cream/90 backdrop-blur-sm border-b border-navy-900/5 sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black text-navy-900 tracking-tight">MedDict</h1>
          </div>
        ...
        <div className="space-y-8">
          ...
          {localResults.length > 0 && (
            <div className="space-y-4 animate-fadeIn">
              ...
              {/* Refine Search / Context Input when local results exist */}
              {!aiLoading && !aiResult && (
                <div className="mt-8 bg-white rounded-xl border-2 border-navy-900/5 p-6 shadow-sm">
                   <div className="mb-4">
                      ...
                      <p className="text-sm text-navy-800/60 font-medium mt-1">
                        输入具体的科室或病历语境，让 AI 提供更精准的解释。
                      </p>
                   </div>
          ...
          {query.trim() && localResults.length === 0 && !aiResult && !aiLoading && !aiError && (
            <div className="bg-white rounded-2xl border-2 border-navy-900/5 p-8 sm:p-12 text-center animate-fadeIn shadow-card">
              ...
              <h3 className="text-2xl font-black text-navy-900 mb-3">本地未收录 "{query}"</h3>
                 ...
                 <div className="text-left bg-navy-50/50 p-2 rounded-xl border border-navy-900/10 focus-within:ring-2 focus-within:ring-navy-900 focus-within:border-transparent transition-all">
                   <div className="relative">
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                       <span className="text-[10px] font-bold text-navy-900/40 border border-navy-900/10 px-2 py-1 rounded bg-white uppercase tracking-wide">Context</span>
                    </div>
                   </div>
                 </div>
          ...
          {aiLoading && (
             <div className="w-full p-12 bg-white/50 backdrop-blur rounded-2xl border border-navy-900/5 text-center">
               ...
               <h3 className="text-xl font-bold text-navy-900">正在咨询专家库...</h3>
             </div>
          )}

          {/* AI Result + Refinement Section */}
          {aiResult && (
            <div className="space-y-6 animate-fadeIn">
               <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-2 rounded-xl border border-navy-900/10 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto p-1">
                     ...
                     <button
                        ...
                        className="flex-1 sm:flex-none flex items-center justify-center px-5 py-2.5 bg-honey-500 hover:bg-honey-600 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-colors gap-2"
                     >
                     ...
                     <button 
                       ...
                       className="px-4 py-2.5 text-navy-900/50 hover:text-navy-900 text-xs font-bold uppercase bg-navy-50 hover:bg-navy-100 rounded-lg transition-colors"
                     >
                       关闭
                     </button>
                  </div>
      ...
      <footer className="relative z-10 py-10 text-center">
        <p className="text-navy-900/30 text-xs font-bold uppercase tracking-widest">© 2024 MedDict</p>
      </footer>
    </div>
  );
}